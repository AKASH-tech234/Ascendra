...
export function CheckInModal({ isOpen, onClose, defaultGoalId }: CheckInModalProps) {
        ...
        <>
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-bold text-ink">Submit Progress Check-in</h3>
              <button
                onClick={onClose}
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              <div className="space-y-2">
                <label className="text-sm font-semibold text-ink">Select Goal</label>
                <select
                ...
                >
                  ...
                  {goals?.filter(g => g.status === "APPROVED" || g.status === "SUBMITTED").map(goal => (
                    <option key={goal.id} value={goal.id}>
                      {goal.title} ({goal.progress}%)
                    </option>
                  ))}
                </select>
                {errors.goalId && <p className="text-xs text-danger-500">{errors.goalId.message}</p>}
                
                {selectedGoal && (
                  <div className="mt-2 flex items-center gap-2 p-3 bg-surface-2/50 rounded-xl border border-line text-sm text-ink-2">
                    <Target className="h-4 w-4 text-accent" />
                    <span>Current Progress: <strong className="text-ink">{selectedGoal.progress}%</strong></span>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <label className="text-sm font-semibold text-ink">New Progress</label>
                  <span className="text-sm font-bold text-accent">{progressValue || 0}%</span>
                </div>
                <input
                  type="range"
                  min="0"
                ...
                />
                {errors.progress && <p className="text-xs text-danger-500">{errors.progress.message}</p>}
              </div>

              <div className="space-y-2">
                <label className="text-sm font-semibold text-ink">Comments & Context</label>
                <textarea
                  ...
                  {...register("comments")}
                />
                {errors.comments && <p className="text-xs text-danger-500">{errors.comments.message}</p>}
              </div>

              <div className="space-y-2">
                <label className="text-sm font-semibold text-ink">Current Blockers (Optional)</label>
                <input
                  type="text"
